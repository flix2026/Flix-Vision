...
function setupEventListeners() {
    ...
    UI.detailsPlayBtn.addEventListener('click', () => {
        ...
        const title = currentDetailItem.title || currentDetailItem.name;
        closeDetailsModal();
        // Track play start
        if (typeof AUTH !== 'undefined') AUTH.trackPlayStart({ title, tmdb_id: currentDetailItem.id, media_type: type });
        playMovie(currentDetailItem.id, type, title, seasonCount);
    });
...
async function switchTab(tabId) {
    ...
    if (typeof AUTH !== 'undefined') AUTH.trackPageView(tabId);
